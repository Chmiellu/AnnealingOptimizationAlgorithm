...
def simulated_annealing(coords, initial_temperature=100, cooling=0.9, computing_time=40):
    dist_matrix = calculate_distance_matrix(coords)
    num_cities = len(coords)
    ...
    while time.time() - start < computing_time:
        new_solution = random.choice([swap_cities, reverse_subsection, insert_city, chmiel_swap])(
            current_solution)
        current_fitness = objective_function(current_solution, dist_matrix)
        new_fitness = objective_function(new_solution, dist_matrix)
